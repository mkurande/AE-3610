...
[c1 c2 c3 c4 c5 c6 c7 c8 c9 c10 c11 c12 c13 c14 c15 c16] = textread('Four Point Bending F2 Data-Stage-0-1.txt','%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f','headerlines',14);
...
%Creating the necesary plots

%truncating the data so its in the regime we want to analyze
YcoordDef = YcoordDef(55:end-20,8:end-5);
XcoordDef = XcoordDef(55:end-20,8:end-5);

Xstrain = Xstrain(55:end-20,8:end-5);
Ystrain = Ystrain(55:end-20,8:end-5);
XYstrain = XYstrain(55:end-20,8:end-5);

coeff = polyfit(YcoordDef,Xstrain,1);
K = -coeff(1);
b = -coeff(2);

plot(YcoordDef,Xstrain);
xlabel('Y');
ylabel('E_x');


%Data Reduction
%Calculating E_Compression and E_Tension
...
E_Tension = M ./ (I .* K .* (1 - (2 .* b) ./ (K .* h)).^2);
E_Compression = M ./ (I .* K .* (1 + (2 .* b) ./ (K .* h)).^2);

%finding poisson ratio
figure();
plot(Xstrain, Ystrain);
coeff2 = polyfit(Xstrain, Ystrain, 1);
xlabel('E_x');
ylabel('E_y');
v = -coeff2(1);
...
%IMAGES for Results
%figure();
%image(XcoordDef,'CDataMapping','scaled');
%colorbar;

%figure();
%image(YcoordDef,'CDataMapping','scaled');
%colorbar;
...
%figure();
%image(XYstrain,'CDataMapping','scaled');
%colorbar;

%Results #5
neutral_surface = -b ./ K;
YcoordDef = YcoordDef - neutral_surface;

yloc0 = YcoordDef(63,:);
Xstrain0 = Xstrain(63,:);
Ystrain0 = Ystrain(63,:);
figure();
plot(yloc0,Xstrain0,'.-k',yloc0,Ystrain0,'.-b');
...
Ystrain1 = Ystrain(10,:);
figure();
plot(yloc1,Xstrain1,'.-k',yloc1,Ystrain1,'.-b');
legend('Strain X','Strain Y');
xlabel('Y');
ylabel('Strain');
